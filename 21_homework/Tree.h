...
private:
	...
	void ShowByBoolFnc(std::function<bool(T)> fcn, Node* root) const;
...
public:
	...
	void ShowByBoolFnc(std::function<bool(T)> fnc) const { (ShowByBoolFnc(fnc, root)); }
...
template<typename T>
inline void Tree<T>::ShowByBoolFnc(std::function<bool(T)> fnc, Node* root) const
{
	if (root == nullptr) return;

	ShowByBoolFnc(fnc, root->left);
	if (fnc(root->data))
		cout << root->data << " ";
	ShowByBoolFnc(fnc, root->right);
}
